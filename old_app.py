...
# Initialize SAM (with graceful fallback if model not available)
...
try:
    if os.path.exists(sam_checkpoint):
        ...
        print("SAM functionality will be disabled. You can download the model from:")
...
def medsam_load_from_supabase():
    """Load image and masks from Supabase database for MedSAM"""
    try:
        ...
        # Use global MedSAM integrator
        global medsam_integrator
        if medsam_integrator is None:
...
# Global MedSAM integrator instance
medsam_integrator = None

@app.route('/api/medsam/get_masks', methods=['POST'])
def medsam_get_masks():
    """Get masks for an image with pagination"""
    try:
        data = request.get_json()
        image_id = data.get('image_id')
        ...
        if image_id not in masks_cache:
            ...
            # Get masks from Supabase (only once per image)
            data = supabase_manager.get_image_and_masks(image_id)
            masks_with_lists = supabase_manager.convert_masks_to_lists(data['masks'])
            
            # Cache the masks
            masks_cache[image_id] = masks_with_lists
            print(f"Cached {len(masks_with_lists)} masks for image {image_id}")
        else:
            # Use cached masks
            masks_with_lists = masks_cache[image_id]
            print(f"Using cached {len(masks_with_lists)} masks for image {image_id}")
        
        # Apply pagination with size limits to prevent JSON serialization issues
        ...
        start_idx = page * page_size
        end_idx = start_idx + page_size
        paginated_masks = masks_with_lists[start_idx:end_idx]
        
        response_data = {
            'success': True,
            'masks': paginated_masks,
            'total_masks': len(masks_with_lists),
            'page': page,
            'page_size': page_size,
            'has_more': end_idx < len(masks_with_lists)
        }
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'error': f'Failed to get masks: {str(e)}'}), 500
...
def medsam_segment_points():
    """Segment using points (each point gets its own small segment)"""
    try:
        data = request.get_json()
        ...
        global medsam_integrator
        if medsam_integrator is None:
            return jsonify({'error': 'MedSAM not initialized. Please load an image first.'}), 500

        if not medsam_integrator.is_available():
            return jsonify({'error': 'MedSAM model not available'}), 500

        if medsam_integrator.current_image is None or medsam_integrator.embedding is None:
            return jsonify({'error': 'No image or embedding loaded for MedSAM'}), 500

        H, W, _ = medsam_integrator.current_image.shape
        box_size = 20  # Small box size for point-based segmentation

        print(f"Processing {len(points)} points for individual segmentation")
        ...
        for i, pt in enumerate(points):
            x, y = pt
            # Create a small bounding box centered on the point
            ...
            if result:
                ...
                print(f"Point {i+1} segmentation successful")
            else:
        ...
        if masks:
            ...
            return jsonify({'error': 'All point segmentations failed'}), 500
...
def medsam_segment_box():
    ...
    try:
        ...
        if not medsam_integrator.is_available():
            print("MedSAM model not available")
            return jsonify({'error': 'MedSAM model not available'}), 500
        
        # Check if image is loaded
        if medsam_integrator.current_image is None:
            print("No image loaded for MedSAM")
            return jsonify({'error': 'No image loaded for MedSAM'}), 500
        
        # Check if embedding is available
        if medsam_integrator.embedding is None:
            print("No embedding available for MedSAM")
            return jsonify({'error': 'No embedding available for MedSAM'}), 500
        
        print(f"Image shape: {medsam_integrator.current_image.shape}")
        print(f"Embedding shape: {medsam_integrator.embedding.shape}")
        ...
        if result:
            ...
            print(f"Segmentation successful, mask shape: {len(result['mask'])}x{len(result['mask'][0])}")
            
            return jsonify({
                'success': True,
                'mask': result['mask'],
                'confidence': result['confidence'],
                'method': result.get('method', 'medsam_box')
            })
        else:
    ...
    try:
        ...
        if result:
            ...
            return jsonify({
                ...
                'mask': result['mask'],
                'confidence': result['confidence'],
                'method': result.get('method', 'medsam_combined')
            })
        else:
            return jsonify({'error': 'Segmentation failed'}), 500
...
def medsam_segment_multiple_boxes():
    """Segment using multiple bounding boxes"""
    try:
        data = request.get_json()
        ...
        for i, bbox in enumerate(bboxes):
            print(f"Processing box {i+1}/{len(bboxes)}: {bbox}")
            result = medsam_integrator.segment_with_box(bbox)
            ...
            if result:
                ...
                confidences.append(result['confidence'])
                print(f"Box {i+1} segmentation successful")
            else:
        ...
        if masks:
            ...
            return jsonify({'error': 'All segmentations failed'}), 500
...
def medsam_status():
    ...
    try:
        ...
        return jsonify({
            ...
            'image_loaded': medsam_integrator.current_image is not None,
            'embedding_loaded': medsam_integrator.embedding is not None
        })
    except Exception as e:
        return jsonify({
            'available': False,
